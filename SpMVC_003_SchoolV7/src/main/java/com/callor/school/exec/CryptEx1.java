 ...
 *  만약 비번을 암호화 한다면
 *  회원가입에서 입력된 비번을 암호화하여 table에 저장하고
 *  이후 로그인을 시도하면 table에 저장된 비번을
 *  다시 복호화하여 로그인 정보와 비교하는 것
 *  
 *  중간에 key가 노출될 수 있고 복호화하는 코드가 공개될 경우
 *  치명적인 보안사고가 날 수 있다
 *  
 * 2. 단방향 암호화
 *  암호화 기능은 존재하지만 복호화 기능이 없는 기법
 *  회원가입할 때 입력된 비번을 암호화하여 table에 저장하고
 *  
 *  이후 로그인을 시도하면
 *  입력된 비번을 다시 암호화하여 table에 저장된 값과
 *  비교한다
 *  복호화 기능이 존재하지 않으므로
 *  암호화 할 때 사용된 기법이 노출되지 않고 다소 안전하게
 *  작동할 수 있다
 *  
 *  암호화하는 방법과 비교하는 방법을 같은 도구를 사용해야 하는
 *  단점이 존재하기도 한다
 *  
 */
