...
public class UserControllerV2 {
	 ...
	 * 사용자가 로그인을 성공하면
	 * 다른 페이지로 이동하였을 때는 그 정보가 유지되어야 한다
	 ...
	 * 관리한다
	 * 로그인한 사용자의 정보(VO 데이터)를 HttpSession에 
	 * 변수로 저장을 해놓는다
	 * session.setAttribute("USER", loginUser); 명령을 사용하여
	 * USER라는 변수에 loginUser 정보를 저장해 놓는다
	 * 이후에 서버를 재시작, 종료하기 전까지 USER라는 변수에는
	...
	public String logout(HttpSession session) {
		 ...
		 * redirect:
		 * 로그아웃이 끝나면
		 * web browser의 주소창에 /user/login을 입력하고
		 * Enter를 눌러라
		 */
		return "redirect:/";
	}
	 ...
	 *  
	 *  id를 email주소로 사용할 때 PathVarriable로 받을 경우
