...
public class AddressServiceImplV1 implements AddressService {
	...
	public void searchAndPage(Model model, SearchPage searchPage) {
		...
		// 1. 전체 데이터 갯수가 몇개인지를 알아야 pagination을 구현할 수 있다
		// 		화면 하단의 페이지 번호를 자동으로 계산하기 위함이다
		// 검색어가 없이 요청이 될 경우 SearchPage 객체의 search 값이 null 이 되어
		// 데이터가 검색이 되지 않는다
		// 때문에 SerarchPage 객체의 search 변수값을 ""으로 세팅
		...
		// 전체 데이터의 총 페이지수
		// 전체 데이터 갯수를 한 페이지에 보여질 데이터 갯수로 나누기
		// 		만약 전체 데이터가 37개이고 한 페이지에 10개를 보여준다면
		// 		finalPageNo는 3이 될 것이다
		long finalPageNo = totalCount / LIST_PER_PAGE;
		// 화면 하단의 페이지번호를 클릭했을 때 전달되는 값
		long currentPageNo = searchPage.getCurrentPageNo();
		// 삼항연산자 사용할 때 -> true 일 때는 ? finalPageNo, false일 때는 : currentPageNo;
