...
public class AdminController {
	...
	/*
	 * Spring security에서 로그인이 성공하면 Principal 이라는 클래스를
	 * 사용할 수 있다
	 * principal.getName() 를 실행하면 로그인한 사용자의 username을
	 * 추출할 수 있다
	 */
	...
	public String userUpdate(UserVO userVO, Principal principal, Model model) {
		...
		if(ret < 0) {
			return String.format("redirect:/admin/userupdate?" + "username=%s&error=ADMIN_PASS", userVO.getUsername());
		}
