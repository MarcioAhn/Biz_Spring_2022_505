/*
 비동기 방식의 코드문제로
 return false 또는 return true가 실행되지 않는 문제를 해결하기 위하여
...
const st_num_fetch = (st_num) => {
  ...
  // fetch가 완료되기 전에 실행되어 st_num_fetch가 종료됨
  // 이 함수가 true 나 false를 return할 것으로 기대하고
  // 작성을 했으나 비동기 방식의 이유로
...
const save_cb = async () => {
  ...
  // st_num_yes 에 true 나 false가 담길것으로 기대하고 있다
  ...
  /*
        ...
        코드 중단을 방지하기 위하여 try-catch로 처리를 해주어야 한다
  
        그런데
        JS에서는 그러한 상황에 Exception이 발생하는 대신
        NaN(Not a Number: 숫자가 아니다)이라는 값을 return한다
